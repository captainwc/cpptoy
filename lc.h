...
#define TODO(msg)                                              \
    do {                                                       \
        GUARD_LOG;                                             \
        std::cerr << ANSI_YELLOW_BG << (msg) << ANSI_PURPLE_BG \
    ...
    do {                                                                \
        ...
        auto microduring =                                              \
            std::chrono::duration_cast<std::chrono::microseconds>(cost) \
                .count();                                               \
        GUARD_LOG;                                                      \
        std::cout << "[" << (n) << " " << (msg) << "]: " << miliduring  \
                  << "ms, (" << microduring / (n) << "us/per)\n";       \
    } while (0);
...
auto Pair2String(const T &p) {
    std::stringstream ss;
    ss << '{' << toString(std::get<0>(p)) << ELEM_SEP
       << toString(std::get<1>(p)) << '}';
    return ss.str();
}
...
inline void parseOperations(
    string operations, const string &datas,
    ...
    Graph(vector<ValueType> v, bool isYouXiangTu = false)
        : nodes(std::move(v)),
          ...
          edges(vector<vector<int>>(
              nodes.size(),
              vector<int>(nodes.size(), graphConst::dummyValue))) {}
