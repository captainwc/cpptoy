...
static_assert(__cplusplus >= 201703L,
              "C++17 is required, which is same as leetcode oj environment");
...
#define COUT(x)                                                                \
  do {                                                                         \
    ...
    GUARD_LOG;                                                                 \
    std::cout << ANSI_BLUE_BG << "[" #x "]: " << ANSI_CLEAR << ret             \
              << std::endl;                                                    \
  } while (0);

#define COUT_POSITION                                                          \
  "[" << __FILE__ << ELEM_SEP << __FUNCTION__ << ELEM_SEP << __LINE__ << "]"
  ...
  do {                                                                         \
    if (expr) {                                                                \
      ...
      std::cerr << ANSI_RED_BG << "[FAILED] " << ANSI_CLEAR                    \
                << "[" + std::to_string(gTotalTest.load()) + "] "              \
                << ANSI_BLUE_BG << #expr << ANSI_RED_BG << " => istrue? "      \
                << ANSI_YELLOW_BG << COUT_POSITION << ANSI_CLEAR << std::endl; \
  ...
  do {                                                                         \
    if ((x) == (y)) {                                                          \
      ...
      std::cout << ANSI_GREEN_BG << "[PASSED] " << ANSI_CLEAR                  \
                ...
                << " == " << ANSI_BLUE_BG << #y << ANSI_CLEAR << std::endl;    \
    } else {                                                                   \
      ++gFailedTest;                                                           \
      GUARD_LOG;                                                               \
      std::cerr << ANSI_RED_BG << "[FAILED] " << ANSI_CLEAR                    \
                << "[" + std::to_string(gTotalTest.load()) + "] "              \
                << ANSI_BLUE_BG << #x << ANSI_RED_BG " == " << ANSI_BLUE_BG    \
                << #y " " << ANSI_YELLOW_BG << COUT_POSITION << ANSI_CLEAR     \
                << std::endl;                                                  \
      std::cerr << ANSI_PURPLE_BG << '\t' << "Expected: " << ANSI_CLEAR        \
                << "Equal" << std::endl;                                       \
      std::cerr << ANSI_PURPLE_BG << '\t' << "  Actual: " << ANSI_CLEAR        \
  ...
  do {                                                                         \
    ...
    if (gFailedTest.load() == 0) {                                             \
      std::cout << ANSI_GREEN_BG << "==== "                                    \
                << std::to_string(gTotalTest.load()) + "/" +                   \
                       std::to_string(gTotalTest.load()) + " PASSED ALL! ====" \
                << ANSI_CLEAR << std::endl;                                    \
    } else {                                                                   \
...
template <typename T>
concept Printable =
    StreamOutable<T> || Serializable<T> || SequentialContainer<T> ||
    MappedContainer<T> || PairLike<T> || StackLike<T> || QueueLike<T>;

template <bool> auto toString(bool obj);

template <Printable T> auto toString(const T &obj);
...
template <PairLike T>
  requires Printable<typename T::first_type> &&
...
inline string TreeNode::toString() { return treeToString(this, ELEM_SEP); }
inline string ListNode::toString() { return listToString(this, ELEM_SEP); }
...
template <bool> auto toString(bool obj) { return obj ? "Ture" : "False"; }
...
template <Printable... Args> void dump(Args... args) {
  ((std::cout << toString(args) << " "), ...);
  std::cout << "\n";
}

template <PairLike... PairType> void dumpWithName(PairType... args) {
  GUARD_LOG;
  ((std::cout << ANSI_BLUE_BG << "[" << toString(std::get<0>(args))
              << "]:" << ANSI_CLEAR << toString(std::get<1>(args)) << " "),
   ...);
}
template <PairLike... PairType> void listWithName(PairType... args) {
  GUARD_LOG;
  ((std::cout << ANSI_BLUE_BG << "[" << toString(std::get<0>(args))
              << "]:" << ANSI_CLEAR << toString(std::get<1>(args)) << "\n"),
   ...);
...
#define DUMP(...)                                                              \
  do {                                                                         \
    GET_MACRO(__VA_ARGS__, DUMP8, DUMP7, DUMP6, DUMP5, DUMP4, DUMP3, DUMP2,    \
              DUMP1)                                                           \
    (__VA_ARGS__);                                                             \
    std::cout << "\n";                                                         \
  } while (0);
...
inline string trimString(string str, string_view trimed = R"( []")") {
  ...
  str.erase(str.begin(),
            std::find_if(str.begin(), str.end(), [&trimedChar](char ch) {
...
inline vector<int> kmpFindAllSubStr(const string &mainStr,
...
inline void
parseOperations(string operations, const string &datas,
  ...
  ASSERT_MSG(ops.size() == das.size(),
             "The nums of Operation and Data is unequal");
  for (int i = 0; i < ops.size(); i++) {
    handler(trimString(ops[i], R"(")"), das[i]);
  }
}

inline void checkOperationsRet(string ops, const string &datas, string expected,
                               string actual) {
  ...
  for (int i = 0; i < vexp.size(); i++) {
    if (vexp[i] != vactual[i]) {
      LOGV("op[{}]: {} {}, Exp {}, Act {}", i, vop[i], vdata[i], vexp[i],
           vactual[i]);
    }
  }
}
...
inline void resetCout() { std::cout.rdbuf(cout_origin_buf); }
...
inline vector<int> getIntVector(string str) {
  ...
  vector<int> ret;
  std::transform(tmp.begin(), tmp.end(), std::back_inserter(ret),
                 [](const string &x) { return std::stoi(x); });
  return ret;
}
...
template <Printable ValueType> class Graph {
public:
...
public:
  ...
  Graph(vector<ValueType> v, bool isYouXiangTu = false)
      : nodes(std::move(v)), isYouXiang(isYouXiangTu), nodeNum(nodes.size()),
        edges(vector<vector<int>>(
            nodes.size(), vector<int>(nodes.size(), graphConst::dummyValue))) {}

  Graph(vector<ValueType> v, vector<vector<int>> e, bool isYouXiangTu = false)
      : nodes(std::move(v)), edges(std::move(e)), nodeNum(nodes.size()),
...
Graph<int> buildRandomGraph(int n = 10, bool isYouXiang = false,
                            bool useDefaultWeight = true, int start = 1,
                            int end = 100) {
...
template <Printable ValueType> string Graph<ValueType>::toString() const {
  stringstream ss;
  ss << "[nodes]:\n\t" << ::toString(nodes) << "\n";
  ss << "[edges]:\n";
  for (int i = 0; i < nodeNum; i++) {
...
template <> string Graph<int>::toString() const {
  if (nodeNum == 0) {
    return "[EMPTY GRAPH]";
  }
  int max = 1;
